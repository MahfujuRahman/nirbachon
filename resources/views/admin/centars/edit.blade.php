...
@section('header', 'Edit Centar')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white shadow-lg rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold">Edit Centar</h3>
        </div>
        <form action="{{ route('admin.centars.update', $centar) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <select name="ashon_id" id="ashon_id" required
                    ...
                    @foreach($ashons as $ashon)
                        <option value="{{ $ashon->id }}" {{ old('ashon_id', $centar->ashon_id) == $ashon->id ? 'selected' : '' }}>
                            {{ $ashon->title }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $centar->title) }}" required
            ...
            <div class="mb-4">
                ...
                <textarea name="address" id="address" rows="3" required
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('address') border-red-500 @enderror">{{ old('address', $centar->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
